...
class Solution(object):
  def qUpdateNextHight(self, liveB, curT):
    if len(liveB) == 0:
      return 0
    while -liveB[0][1] <= curT:
      heapq.heappop(liveB)
      if len(liveB) == 0:
        return 0
    return -liveB[0][0]
  ...
  def getSkyline(self, buildings):
    ends = [[b[1], b[2]] for b in buildings]
    ends = sorted(ends, key = lambda x: x[0])
    liveB, curH, curT, res = [], 0, 0, []

    while len(ends) != 0:
      if len(buildings) == 0:
        curT, h = ends.pop(0)
        nextH = self.qUpdateNextHight(liveB, curT)
        if nextH < curH:
          curH = nextH
          res += [[curT, curH]]

      elif buildings[0][0] < ends[0][0]: #the next is starting a building
        curT, e, h = buildings.pop(0)
        heapq.heappush(liveB, (-h, -e))
        while len(buildings) != 0 and buildings[0][0] == curT:
          curT, e, hnext = buildings.pop(0)
          heapq.heappush(liveB, (-hnext, -e))
          h = max(h, hnext)
        if h > curH:
          curH = h
          res += [[curT, curH]]

      elif buildings[0][0] > ends[0][0]: #the next is ending a building
        curT, h = ends.pop(0)
        nextH = self.qUpdateNextHight(liveB, curT)
        if nextH < curH:
          curH = nextH
      ...
      elif buildings[0][0] == ends[0][0]:
        curT, e1, h1 = buildings.pop(0)
        curT, h2 = ends.pop(0)
        heapq.heappush(liveB, (-h1, -e1))
        nextH = self.qUpdateNextHight(liveB, curT)
        if nextH < curH:
          curH = nextH
          res += [[curT, curH]]
